    ...
    for(var i=0,j=0;i<pict_nums.length,j<pict_nums.length;i++,j++){
        ...
        !function(marker,lnglats,i){
            ...
            marker.addEventListener("click",function(){
                var point = lnglats;
                var html='';
                html+="地块编号："+infos[i].landId+
                      "<br/>土地坐落："+infos[i].landPosition+
                      "<br/>土地性质："+infos[i].landNature+
                      "<br/>出让面积："+infos[i].landArea+"m²"+
                      "<br/>起始价："+infos[i].startPrice+"万元"+
                      "<br/>成交价："+infos[i].currentPrice+"万元"+
                      "<br/>竞得人："+infos[i].competitionMan+
                      "<br/>出让时间："+infos[i].sellTime+"。";
